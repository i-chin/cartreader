...
void readROM_LOOPY() {
  ...
  sprintf_P(calculatedChecksumStr, PSTR("%08lX"), sum);
...
void writeSRAM_LOOPY() {
  ...
  char str[16];
  sprintf_P(str, PSTR("Writing %ld bytes to"), sramSize);
  println_Msg(str);
...
void readSRAM_LOOPY() {
  ...
  sprintf_P(fileName, PSTR("%s.sav"), romName);

  EEPROM_readAnything(FOLDER_NUM, foldern);
  sprintf_P(folder, PSTR("LOOPY/SAVE/%s/%d"), romName, foldern);
  sd.mkdir(folder, true);
  sd.chdir(folder);
