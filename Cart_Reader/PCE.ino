...
static const char pceMenuItem3[] PROGMEM = "HuCARD Flash Repro";
static const char pceMenuItem4[] PROGMEM = "AdapterSetting";
...
uint8_t read_byte_PCE(uint32_t address) {
  ...
  if (pce_internal_mode == HUCARD || pce_internal_mode == PCE_FLASH || pce_internal_mode == TURBOCHIP_NOSWAP) {
...
void write_byte_PCE(uint32_t address, uint8_t data) {
  ...
  //Swap bit order for PC Engine HuCARD
  if (pce_internal_mode == HUCARD || pce_internal_mode == TURBOCHIP_NOSWAP || pce_internal_mode == PCE_FLASH) {
