...
setrom:
  ...
  Serial.println(F("KB"));
#endif
  EEPROM_writeAnything(TRS80_SIZE, newtrs80size);
  trs80size = newtrs80size;
}

void checkStatus_TRS80()
{
  EEPROM_readAnything(TRS80_SIZE, trs80size);
  if (trs80size > trs80hi) {
    trs80size = 5; // default 32K
    EEPROM_writeAnything(TRS80_SIZE, trs80size);
  }
...
void setCart_TRS80()
{
  ...
  if (myFile.open("trs80cart.txt", O_READ)) {
    ...
    if(checkCartSelection(myFile, &readDataLineSingleDigit, &gameSize)) {
      EEPROM_writeAnything(TRS80_SIZE, gameSize);
    }
  } else {
