...
setrom:
  ...
  Serial.println(F("KB"));
#endif
  EEPROM_writeAnything(VECTREX_ROM_SIZE, newvectrexsize);
  vectrexsize = newvectrexsize;
}

void checkStatus_VECTREX() {
  EEPROM_readAnything(VECTREX_ROM_SIZE, vectrexsize);
  if (vectrexsize > 2) {
    vectrexsize = 0;  // default 4KB
    EEPROM_writeAnything(VECTREX_ROM_SIZE, vectrexsize);
  }
...
void setCart_VECTREX() {
  ...
  if (myFile.open("vectrexcart.txt", O_READ)) {
    ...
    if(checkCartSelection(myFile, &readDataLineSingleDigit, &gameSize)) {
      EEPROM_writeAnything(VECTREX_ROM_SIZE, gameSize);
    }
  } else {
