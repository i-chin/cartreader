...
void setCart_Jag() {
  ...
  if (myFile.open("jag.txt", O_READ)) {
    ...
    if(checkCartSelection(myFile, &readDataLine_Jag, &entry, &printDataLine_Jag,&setRomnameFromString)) {
      EEPROM_writeAnything(ATARI_JAGUAR_EEP_SIZE, entry.saveSize);
      jagEepSize = entry.saveSize;
      switch (entry.gameSize) {
        case 1:
          jagromSize = 0;
          EEPROM_writeAnything(ATARI_JAGUAR_ROM_SIZE, jagromSize);
...
void jagEepMenu() {
  ...
  switch (subMenu) {
    case 0:
      jagEepSize = 0; // 128B
      EEPROM_writeAnything(ATARI_JAGUAR_EEP_SIZE, jagEepSize);
...
void setup_Jag() {
 ...
 EEPROM_readAnything(ATARI_JAGUAR_ROM_SIZE, jagromSize);
  EEPROM_readAnything(ATARI_JAGUAR_EEP_SIZE, jagEepSize);
  // Print all the info
  if (jagromSize > 2) {
